...
ONMS_PASS="$(cat ../.opennms.env | grep GRAFANA | awk -F "=" '{ print $2 }')"
ONMS_USER=grafana
...
curl --basic -PUT "$GF_USER:$GF_PASS@localhost:3000/api/plugins/opennms-helm-app/settings?enabled=true" -d '' 2>/dev/null
if [ $? -eq 0 ]; then
    echo -e "${GREEN}done${NO_COLOR}"
else
    echo -e "${RED}failed${NO_COLOR}"
